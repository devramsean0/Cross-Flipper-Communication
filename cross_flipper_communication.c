...
// Custom Event Enums
typedef enum { AppSubGHzSelectFrequency, AppSelectModeSubGhz } AppCustomEvents;
...
bool app_scene_subghz_select_frequency_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        consumed = true;
        scene_manager_next_scene(app->scene_manager, AppSceneSubGhzChatSelectMode);
    }
    return consumed;
};
